  ...
  public void run() {
    ...
    if (sMethod.equalsIgnoreCase("POST"))
	  ...
	  head();
  		...
  		throw new RuntimeException(use.getClass().getName()+" "+use.getMessage(), use);
  	} catch (IOException ioe) {
  		throw new RuntimeException(ioe.getClass().getName()+" "+ioe.getMessage(), ioe);
  	}
  } // run
  ...
  public Object post ()
	...
	if (aParams!=null) {
	  for (int p=0; p<aParams.length; p++) {
	    sParams += aParams[p].getKey()+"="+URLEncoder.encode(aParams[p].getValue(), "ISO8859_1");
